<?php
	...
	if (isset($_GET['pagina']))
		...
		$pagina=0;
	Conectar();
	$sql="SELECT * FROM compras WHERE estado_compra='Pendiente'";
	...
	$sql1="SELECT * FROM compras INNER JOIN clientes on compras.clientes_id_cliente = clientes.id_cliente WHERE estado_compra='Pendiente' LIMIT ".$inicio.",".$MaxReg;
?>
	...
	<body class="sb-nav-fixed" >
		...
		<div id="layoutSidenav" style="margin:2em; margin-left: 3em; margin-right: 3em">
			<div id="layoutSidenav_content">
				<main>
					<h4><center>Compras Pendientes</center></h4><br/>
					<div >
					...
					<div class="table-responsive">
										...
										{?>
											<tr>
												<form action="procesarAdmin.php" method="post">
													...
													<td><input class="btn btn-primary" style='background-color:#12bfb5; border-color:#12bfb5; width:105px' type="submit" value="Atender"></td>
													<input type="hidden" name="id_compra" value="<?= $fila["id_compra"] ?>">
													<input type="hidden" name="correoCliente" value="<?= $fila["correo_cliente"]  ?>">
													<input type="hidden" name="valorTotal" value="<?= $fila["precioFinal_compra"] ?>">
													<input type="hidden" name="txtAccion" value="modificarAtendido">
												</form>
											</tr>
											<?php
										}
						...
						<ul class="pagination justify-content-center"><?php Paginamiento($pagina,$totalPaginas, $MaxReg, "VisualizarComprasPendientes.php?")?></ul>
					</div><br/>
				</main>
			</div>
		</div>
